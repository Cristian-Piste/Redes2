...
void crear_cubeta (datos * Estructuras, int num_cubetas, int * numeros, int port);
...
{
	if(argv < 3){
		perror("./client [#CUBETAS][SERVER_PORT]\n");
		exit(0);
  	}

	int i, test, num_cubetas = atoi (argc [1]), numeros [ARRAY_LENGTH], port = atoi (argc [2]);
	...
	crear_cubeta (Estructuras, num_cubetas, numeros, port);
...
void crear_cubeta (datos * Estructuras, int num_cubetas, int * numeros, int port)
{
	...
	for (i = 0; i < num_cubetas; i ++)
	{
		...
		Estructuras [i].host_port = (port + i);
